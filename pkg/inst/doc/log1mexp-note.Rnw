...
\Abstract{In this note, we explain how $f(a) = \log(1 - e^{-a}) =\log(1 -
  ...
  The branch cutoff, $a_0$, in use in \R{} since % version 1.9.0, April
  2004, is shown to be optimal both empirically and theoretically.
}
...
language standards around the same time, e.g., \citet{ieee04:log1p}.
...
\mathrm{and}\\
  ...
  \ \mathrm{for}\ \ \abs{x} < 1,
\end{align}
and $n! = n(n-1)\cdots 1$ denotes factorial.
...
even for the (inverse logit) link function in logistic regression, see, e.g.,
...
we have defined the macro \verb|R_D_LExp(x)| to provide these two branches
...
smaller $a$ ($a$ smaller than about 710, here).% < \approx 710$, here.
...
Below, we zoom in the region where all methods have about of the same
(good) accuracy.
...
<<bigpict-fig, fig=TRUE, grdevice = pdfaCrop, echo=FALSE, height=5, width=7>>=
<<bigpict-setup>>
<<bigpict-def>>
...
\begin{figure}[hbt!]
\centering
<<zoom-in-fig, fig=TRUE, grdevice= pdfaCrop, height=5, width=8, echo=FALSE>>=
...
\begin{align}
  ...
  \end{cases}
\end{align}
The mathematical argument for chosing $a_0$ is quite simple:
In which situations does $1 - e^{-a}$ loose any digits?
Well, as soon as $e^{-a} < \frac 1 2 = 2^{-1}$, because then, at least one
bit cancels. But that is equivalent to $-a < \log(2^{-1}) = -\log(2)$ or
$a > \log(2) =: a_0$.
