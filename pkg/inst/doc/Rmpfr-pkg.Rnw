...
in combinatorics or number theory, you may occasionaly want to
work with \emph{exact} binomial coefficients, e.g., you may need a full row
of Pascal's triangle, i.e., want all $\binom{n}{k}$ for $n=80$ (and hence $k=0,\dots,n$),
which with \pkg{Rmpfr} can be as simple as
<<chooseM-ex>>=
...
- Why S4 classes and methods are useful here:
