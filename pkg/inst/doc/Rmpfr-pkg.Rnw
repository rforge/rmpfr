...
See \url{http://mpfr.org/}.

MPFR itself is built on and requires the
GNU Multiple Precision arithmetic library (GMP),
see \url{http://gmplib.org/}. It can be obtained from there,
...
## a more precise version of  k/7,  k = 0...7  from above
...
@
and then the same, using 128-bit accurate mpfr-numbers:
<<integ-exp-mpfr>>=
