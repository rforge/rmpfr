...
\subsection{Mathematical Constants, Pi ($\pi$), gamma, etc}
...
\subsection{\code{seqMpfr()} for sequences:}
...
convenient, but straightforward to autmatically generalize for mpfr numbers.
Instead, we provide the \code{seqMpfr} function...

\subsection{Rounding, \code{roundMpfr()}, \code{asNumeric()} etc:}
...
Often an alternative is to ``round to double precision'' producing standard
...
The CRAN package \CRANpkg{Bessel}
