...
%___TODO: Set of experiments (with smaller Sweave file)___
...
overflows numerically\footnote{Indeed, $\log(1 - F_L(x)) = -g(x)$, for $x=800$,
  \code{plogis(800, lower=FALSE, log.p=TRUE)},
  underflowed to \code{-Inf} in \R, before version 2.15.1 (June 2012) from
...
where have used \code{uniroot()} to determine the zero crossing, but will
use slightly simpler cutoffs $x_1$, $x_2$, in (\ref{eq:log1pexp}) below:
%% Notation x_1, x_2  are first and 2nd cutoff in equation (10)
<<uniroot-x1>>=
...
\begin{align}
  ...
  \begin{cases}
    \loglp(\exp(x))     & x \le  x_1 := 18, \\
    x + \exp(-x))       & x_1 < x \le x_2 := 33.3, \\
    x                   & x > x_2,
  \end{cases}
\end{align}
where $x_2$ could be replaced by a larger number such as $34$ (and even
$100$, but not $800$\footnote{The \R{} plot
  \code{curve(log1p(exp(x)) - x, 33.1, 33.5, n=2\^{}10)}
  reveals a somewhat fuzzy cutoff $x_1$})., still do a little analysis --- invisibly mostly (echo=FALSE) here
...
$\log1p\bigl(e^x\bigr)$, the intermediate approximation $x + e^{-x}$, and
...
<<bigpict-2-fig, fig=TRUE, echo=FALSE, height=5>>=
...
\setcapwidth{\textwidth}%
  \caption{Absolute relative errors (with respect to 1024 bit \pkg{Rmpfr}
    computation) of four different ways to numerically compute
    $\log\bigl(1 + e^{x}\bigr)$.  Vertical bars at $x_1 = 18$ and $x_2 =
    33.3$ visualize the cutpoints of (\ref{eq:log1pexp}).  Note that
    the default method is fully accurate on this $x$ range.}
  \label{fig:log1pexp}
\end{figure}
